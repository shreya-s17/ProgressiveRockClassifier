...
class SelfAttention(nn.Module):
    def __init__(self, hidden_size, context_size, context_hops=1):
        super(SelfAttention, self).__init__()
        self.W1 = nn.Linear(hidden_size, context_size)
        self.tan = nn.Tanh()
        self.W2 = nn.Linear(context_size, context_hops)

    def forward(self, x):
        # x is rnn output: (B x T x H)
        # (B x A x H) * (B x T x H)^T = (B x T x A)
        x = self.W1(x)
        # tanh(B x T x A)
        x = torch.tanh(x)
        # This represents context hops, r. For 1 sequence-wide hop, it is essentially vector ws2
        # (B x r x A) * (B x T x A) = (B x T x r)
        x = self.W2(x)
        A = F.softmax(x, dim=1)
        return A
...
# Borrowed from deepspeech.pytorch, @SeanNaren on GitHub
# Modified to operate on 2D instead of 3D spectograms
